...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function saveData() {
    const data = {
      ...
      boosters: Array.from(document.getElementsByClassName("booster")).map(
        (booster) => ({
          percent: booster.querySelector(".booster-percent").value,
          days: Array.from(
            booster.querySelectorAll(".day:checked")
          ).map((c) => c.value),
    ...
    render() {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        for (const [entryDateString, times] of entriesArray) {
          ...
          if (
            stripped >= this.stripTime(shiftStart) &&
            stripped <= this.stripTime(shiftEnd)
          ) {
  ...
  function calculateWage() {
    ...
    // Gather boosters
    const boosters = Array.from(document.getElementsByClassName("booster")).map(
      (booster) => ({
        percent: parseFloat(
          booster.querySelector(".booster-percent").value
        ),
        days: Array.from(
          booster.querySelectorAll(".day:checked")
    ...
    const timeEntries = Object.entries(calendar.entries).map(
      ([dateString, times]) => {
          ...
          ${
            boostDetails.length
              ? `<p>Boosts Applied: ${boostDetails
                  .map(
                    (d) =>
                      `${d.percent}% on ${d.hours}h (entry ${d.date})`
                  )
                  .join(", ")}</p>`
              : ""
          }
